  ...
  return <div className="min-h-screen flex flex-col">
      ...
      <section className="relative overflow-hidden py-20 px-6">
        ...
        <div className={`container mx-auto text-center relative z-10 transition-opacity duration-700 ${loaded ? 'opacity-100' : 'opacity-0'}`} id="main-content" tabIndex={-1}>
          ...
          {/* Category Filters - Updated with square rounded appearance and more spacing */}
          <div className="flex flex-wrap justify-center gap-8 stagger-animate mt-12">
      ...
      <section className="container mx-auto px-6 py-12 -mt-8 relative z-10 bg-white rounded-t-3xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8 stagger-animate">
          {filteredStores.map(store => <StoreCard key={store.id} id={store.id} title={store.title} storeName={store.storeName} description={store.description} category={store.category} imageUrl={store.imageUrl} offer={store.offer} giveaway={store.giveaway} experience={store.experience} className="animate-slide-in" price={store.price} duration={store.duration} />)}
        </div>
